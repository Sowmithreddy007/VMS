...
// Get event_id for Nostalgia
$event_name = 'Nostalgia';
$event_query = mysqli_query($conn, "SELECT event_id FROM tbl_events WHERE event_name = '$event_name'");
$event = mysqli_fetch_assoc($event_query);
$event_id = $event['event_id'] ?? null;

if (!$event_id) {
    die("Event 'Nostalgia' not found. Please create the event first.");
}

// ================= Dashboard Stats =================
$total_visitors = mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM tbl_visitors WHERE event_id = $event_id"));
$checked_in = mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM tbl_visitors WHERE event_id = $event_id AND status=1 AND in_time IS NOT NULL"));
$goodies_given = mysqli_fetch_row(mysqli_query($conn, "SELECT SUM(g.quantity) FROM tbl_goodies_distribution g JOIN tbl_visitors v ON g.visitor_id = v.id WHERE v.event_id = $event_id"));
$event_participants = mysqli_fetch_row(mysqli_query($conn, "SELECT SUM(participant_count) FROM tbl_event_participation WHERE event_id = $event_id"));

// ================= Visitor Data =================
$visitors = mysqli_query($conn, "SELECT * FROM tbl_visitors WHERE event_id = $event_id ORDER BY created_at DESC LIMIT 10");

// ================= Inventory Data =================
$inventory = mysqli_query($conn, "SELECT * FROM tbl_inventory ORDER BY status, item_name");

// ================= Goodies Distribution =================
$goodies = mysqli_query($conn, "SELECT g.*, v.name as visitor_name FROM tbl_goodies_distribution g LEFT JOIN tbl_visitors v ON g.visitor_id = v.id WHERE v.event_id = $event_id ORDER BY g.distribution_time DESC LIMIT 10");

// ================= Event Participation =================
$participation = mysqli_query($conn, "SELECT * FROM tbl_event_participation WHERE event_id = $event_id ORDER BY participant_count DESC");

// ================= Coordinator Notes =================
$notes_log = mysqli_query($conn, "SELECT * FROM tbl_coordinator_notes WHERE note_type='LOG' AND event_id = $event_id ORDER BY created_at DESC LIMIT 5");
$notes_actions = mysqli_query($conn, "SELECT * FROM tbl_coordinator_notes WHERE note_type='ACTION_ITEM' AND event_id = $event_id ORDER BY created_at DESC LIMIT 5");
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <title>VMS Console — Nostalgia Dashboard</title>
...
<body>
  ...
  <aside class="sidebar" id="sidebar">
    ...
    <ul class="nav">
      <li><a href="admin_dashboard.php"><i class="fa-solid fa-house"></i><span>Admin Dashboard</span></a></li>
      ...
      <li><a href="nostalgia_dashboard.php" class="is-active"><i class="fa-solid fa-scroll"></i><span>Nostalgia</span></a></li>
      <li><a href="alumni_talks_dashboard.php"><i class="fa-solid fa-microphone-lines"></i><span>Alumni Talks</span></a></li>
      <li><a href="induction_program_dashboard.php"><i class="fa-solid fa-graduation-cap"></i><span>Induction Program</span></a></li>
      <li><a href="mock_interviews_dashboard.php"><i class="fa-solid fa-briefcase"></i><span>Mock Interviews</span></a></li>
    </ul>
  </aside>
  ...
  <main class="main">
    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-3">
      <div class="title-row">
        ...
        <h2>📊 VMS — Nostalgia Dashboard</h2>
        <span class="badge">Event-Specific Data</span>
      </div>
