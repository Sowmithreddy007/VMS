 ...
 * @param int $year Visitor's year of graduation
 * @param string $branch Visitor's branch
 * @return bool|int Result of insertion or false on failure
 */
function create_visitor($name, $roll_number, $year, $branch) {
    ...
    // Set registration type to 'spot' for spot registrations
    ...
    $data = array(
        ...
        'year_of_graduation' => $year, // Fixed key name
        'branch' => $branch,
...
function update_visitor($visitor_id, $updated_data) {
    ...
    $stmt = $conn->prepare("SELECT added_by FROM tbl_visitors WHERE id = ?");
    ...
    try {
        ...
        foreach ($updated_data as $field => $value) {
            if (in_array($field, ['event_id', 'name', 'email', 'mobile', 'address', 'department', 'gender', 'year_of_graduation', 'relation', 'visitor_type', 'registration_type'])) {
        ...
        $sql = "UPDATE tbl_visitors SET " . implode(", ", $set_parts) . " WHERE id = ?";
        $types .= 'i';
        $values[] = $visitor_id;
